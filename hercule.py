...
class Request:
	...
	def __init__(self, api_key):
		self.max_names = 40
		self.prior = 'http://prod.api.pvp.net/api/'
		self.version = ['/v1.1', '/v2.1']
		self.api_key = str(api_key)

	def get_id_from_name(self, name, region='na'):
		request_string = 'lol/' + region + self.version[0] + '/summoner/by-name/' + name 
	...
	def get_summoner_from_id(self, summoner_id, region='na'):
		request_string = 'lol/' + region + self.version[0] + '/summoner/' + str(summoner_id)
		return self.make_request(request_string)
	...
	def get_names_from_ids(self, summoner_ids, region='na'):
		...
		for id_list in [summoner_ids[i:i+self.max_names] for i in range (0, len(summoner_ids), self.max_names)]:
			request_string = 'lol/' + region + self.version[0] + '/summoner/' + ','.join(map(str, id_list)) + '/name'
	...
	def get_teams_from_id(self, summoner_id, region='na'):
		request_string = region + self.version[1] + '/team/by-summoner/' + str(summoner_id) 
		return self.make_request(request_string)
